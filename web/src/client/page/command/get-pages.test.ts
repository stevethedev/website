...
import { GetPagesCommand } from "@/client/page/command/get-pages";
...
describe("GetPagesCommand", () => {
  ...
  it("should fetch pages with filter", async () => {
    ...
    const getPages = new GetPagesCommand({ filter: { path: "/test" } });
  ...
  it("should fetch pages with limit", async () => {
    ...
    const getPages = new GetPagesCommand({
      filter: { path: "/test" },
      limit: 1,
    });
    const { payload } = await getPages.execute(config);
